...
var Server = function (options) {
  ...
  var callHandler = function (res, requests) {
    ...
    if(requests.length > res.rpc.idx) {
      ...
      if (conf.strict && (r.jsonrpc !== '2.0' || !r.id))
        ...
        if (typeof conf.functions[r.method.toLowerCase()] === 'function')
          conf.functions[r.method.toLowerCase()](r.params, function (err, ret) {
            var reply = setReply(ret, err, r.id);
            if (reply) res.rpc.batch.push(reply);
            callHandler(res, requests);
          }, r.method.toLowerCase(), res.__geturi, res.__rawbody);
        else
  ...
  this.addMethod = function (name, func) {
    if (name && typeof func === 'function')
      conf.functions[name.toLowerCase()] = func;
  };

  this.delMethod = function (name) {
    if (conf.functions[name])
      conf.functions[name.toLowerCase()] = null;
  };

  this.start = function (callback) {
    try {
	  serv.timeout = conf.timeout || 120000;
